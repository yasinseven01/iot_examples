// Firebase Realtime Databaseden okuduğumuz sıcaklık verisine göre kombiyi aç kapa
//********************************************************
...
void loop() 
{
  sicaklik_oku_yaz();   // firebase sicakligi yaz
  float turn_on=Firebase.getFloat("TurnOn");
  if(sicaklik<turn_on)
  {
    Serial.println("Kombi acik");
    digitalWrite(D0,HIGH);    // Kombi ON
  }
  else
  {
    Serial.println("Kombi kapali");
    digitalWrite(D0,LOW);     // Kombi OFF
  }
  delay(5000);
}
//************************************************************
